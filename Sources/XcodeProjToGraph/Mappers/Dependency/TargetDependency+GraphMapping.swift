...
extension TargetDependency {
  ...
  public func graphDependency(
    ...
    switch self {
    case .target(let name, let status, _):
      return .target(name: name, path: sourceDirectory, status: status)

    case .project(let targetName, let projectPath, let status, _):
    ...
    case .framework(let path, let status, _):
      let binaryPath = path.appending(component: "\(self.name)")
    ...
    case .xcframework(let path, let status, _):
      return .xcframework(
    ...
    case .library(let path, let publicHeaders, let swiftModuleMap, _):
    ...
    case .package(let product, let type, _):
    ...
    case .sdk(let name, let status, _):
      return .sdk(
        name: name,
        path: sourceDirectory,
        status: status,
