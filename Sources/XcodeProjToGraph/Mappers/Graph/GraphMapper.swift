...
/// - `.project(AbsolutePath)`: Constructs a graph from a single project located at the given path, treating it as a workspace with one project.
...
public final class GraphMapper: Sendable {
  ...
  /// Constructs an `XcodeGraph.Graph` by mapping all projects, packages, and dependencies within the specified workspace or project.
  ///
  ...
  public func xcodeGraph() async throws -> XcodeGraph.Graph {
    ...
    let workspace =
      switch graphType {
      case .workspace(let workspaceProvider):
    ...
    for (path, project) in projects {
      ...
      for (name, target) in project.targets {
        ...
        let targetDependencies = edgesAndDependencies.compactMap { $0.2 }
